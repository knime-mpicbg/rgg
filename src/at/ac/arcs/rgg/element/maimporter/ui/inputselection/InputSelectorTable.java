...
/**
 * A component to allow interactive customization of <code>JXTable</code>'s columns. It's main purpose is to allow
 * toggling of table columns' visibility. Additionally, arbitrary configuration actions can be exposed.
 * <p/>
 * <p/>
 * This component is installed in the <code>JXTable</code>'s trailing corner, if enabled:
 * <p/>
 * <pre><code>
 * table.setColumnControlVisible(true);
 * </code></pre>
 * <p/>
 * From the perspective of a <code>JXTable</code>, the component's behaviour is opaque. Typically, the button's action
 * is to popup a component for user interaction. <p>
 * <p/>
 * This class is responsible for handling/providing/updating the lists of actions and to keep all action's state in
 * synch with Table-/Column state. The visible behaviour of the popup is delegated to a
